...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        if User.query.filter_by(email=data["email"]).first():
            flash("You're already registered with this email. Login instead")
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if not user:
            ...
            flash("The password you typed seems to be wrong. Please try again")
            return redirect(url_for('login'))
        else:
            login_user(user)
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You need to login to save comments")
            return redirect(url_for('login'))
        else:
            new_comment = Comment()
            for field in comment_form.data:
                setattr(new_comment, field, comment_form.data[field])
